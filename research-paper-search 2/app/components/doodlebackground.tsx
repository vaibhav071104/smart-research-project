...
export default function DoodleBackground() {
  ...
  useEffect(() => {
    ...
    doodlesRef.current = Array.from({ length: 12 }, () => ({
    ...
    const drawCap = (ctx: CanvasRenderingContext2D, size: number) => {
      ...
      ctx.beginPath()
      ctx.moveTo(0, -size / 3)
      ctx.quadraticCurveTo(size / 3, -size / 6, size / 3, size / 6)
      ctx.stroke()
    }
    ...
    const animate = () => {
      ...
      doodlesRef.current.forEach((doodle) => {
        // Update rotation
        doodle.rotation += doodle.speed

        // Update opacity for fade effect
        doodle.opacity += 0.003 * doodle.opacityDirection
